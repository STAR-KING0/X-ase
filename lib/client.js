...
async function MakeSession() {
    if (!fs.existsSync(__dirname + '/auth_info_baileys/creds.json')) {
        const sessionData = Config.sessionName; // Fetch session from config.js
        const sessionBuffer = Buffer.from(sessionData, 'base64').toString('utf8');
        await fs.writeFileSync(__dirname + '/auth_info_baileys/creds.json', sessionBuffer, "utf8");
    }
}
